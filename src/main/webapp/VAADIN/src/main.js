...
var Rappid = Backbone.Router.extend({
    ...
    initializePaper: function() {
        ...
        $('.paper-scroller').on('mousewheel DOMMouseScroll', _.bind(function(evt) {
        	var offset = $('.paper-scroller').offset();
            evt.preventDefault();
            var delta = Math.max(-1, Math.min(1, (evt.originalEvent.wheelDelta || -evt.originalEvent.detail)));
            delta = delta/100;
            var paperX = evt.clientX-offset.left;
            var paperY = evt.clientY-offset.top;
            this.zoom((this.zoomLevel || 1) + delta, paperX, paperY);
    ...
    initializeStencil: function(stencilSize, stencilColumns, stencilCaption) {

    	//Groups parameter is not passed in this project (we don't want to use grouping in stencil)
        this.stencil = new joint.ui.Stencil({ graph: this.graph, paper: this.paper, width: stencilSize });
    ...
    zoom: function(newZoomLevel, ox, oy) {
        ...
        if(ox === undefined){
        	ox = (this.paper.el.scrollLeft + this.paper.el.clientWidth / 2) / this.zoomLevel;
        	oy = ox;
        }else{
